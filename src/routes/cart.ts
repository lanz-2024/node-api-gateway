...
import type { Cart, CartItem, AddCartItemBody, UpdateCartItemBody } from '../types/index.js';
    ...
    async (c) => {
      ...
      if (useMock) {
        ...
        const item: CartItem = {
          ...
          price: unitPrice,
          line_total: (parseFloat(unitPrice) * body.quantity).toFixed(2),
        };
  ...
  router.put(
    ...
    async (c) => {
      ...
      if (useMock) {
        ...
        item.line_total = (parseFloat(item.price) * body.quantity).toFixed(2);
        recalcCart(cart);
